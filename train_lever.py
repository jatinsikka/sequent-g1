"""PPO training for the LEVER task (LeverEnv). Mirrors train_push.py.

Run:  python train_lever.py --tag lever-v0 --timesteps 1500000 --n_envs 56
Smoke: python train_lever.py --smoke --no_subproc
"""
...
from lever_env import LeverEnv
...
def make_env(rank: int, seed: int = 0):
    def _f():
        env = LeverEnv(); env.reset(seed=seed + rank); return env
    return _f


class RateLogger(BaseCallback):
    def __init__(self): super().__init__(); self.reset_acc()
    def reset_acc(self): self.n = 0; self.succ = 0; self.err_sum = 0.0; self.ang_sum = 0.0
    def _on_step(self) -> bool:
        for info in self.locals["infos"]:
            self.n += 1
            self.succ += int(info.get("success", False))
            self.err_sum += float(info.get("angle_err", 0.0))
            self.ang_sum += float(info.get("lever_angle", 0.0))
        return True
    def _on_rollout_end(self) -> None:
        if self.n:
            self.logger.record("lever/success_frac", self.succ / self.n)
            self.logger.record("lever/mean_err", self.err_sum / self.n)
            self.logger.record("lever/mean_angle_deg", np.degrees(self.ang_sum / self.n))
        self.reset_acc()


def evaluate(model, n_episodes=20):
    env = LeverEnv(); succ = 0; final_a = []; final_e = []
    for ep in range(n_episodes):
        obs, _ = env.reset(seed=20_000 + ep); s = False; done = False; ang = 0.0; e = 0.0
        while not done:
            a, _ = model.predict(obs, deterministic=True)
            obs, r, term, trunc, info = env.step(a)
            s = s or info["success"]; ang = info["lever_angle"]; e = info["angle_err"]; done = term or trunc
        succ += int(s); final_a.append(ang); final_e.append(e)
    env.close()
    return {"success": succ / n_episodes,
            "mean_final_angle_deg": float(np.degrees(np.mean(final_a))),
            "mean_final_err_rad": float(np.mean(final_e))}


def main():
    p = argparse.ArgumentParser()
    p.add_argument("--tag", default="lever-v0")
    ...
    a = p.parse_args()
    if a.smoke:
        a.timesteps = 8000; a.n_envs = 4; a.tag = "lever-smoke"
